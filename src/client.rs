...
pub fn run_remote(terminal: &mut Terminal<CrosstermBackend<crate::platform::PsmuxWriter>>, input: &crate::ssh_input::InputSource) -> io::Result<()> {
    ...
    // Track paste_pend growth to detect ongoing paste during stage2 timeout.
    #[cfg(windows)]
    let mut last_paste_pend_len: usize = 0;
    // After any non-paste_pend send-paste (right-click clipboard, etc.),
    // suppress paste_pend to avoid duplicate sends from ConPTY key echoes.
    #[cfg(windows)]
    ...
    loop {
        ...
        #[cfg(windows)]
        {
            // If a send-paste was already sent (right-click clipboard),
            // discard ConPTY echo chars that duplicate the content.
            if let Some(until) = paste_suppress_until {
                if Instant::now() < until {
                    if !paste_pend.is_empty() {
                        if input_log_enabled() {
                            input_log("paste", &format!(
                                "suppressing {} duplicate chars (right-click paste active)", paste_pend.len()));
                        }
                        paste_pend.clear();
            ...
            if let Some(start) = paste_pend_start {
                ...
                if paste_confirmed {
                    // Ctrl+V Release already seen — promote to stage2 and
                    // let growth detection wait for ConPTY to finish.
                    if !paste_pend.is_empty() {
                        if input_log_enabled() {
                            input_log("paste", &format!("paste CONFIRMED (top), promoting {} chars to stage2", paste_pend.len()));
                        }
                        paste_stage2 = true;
                        last_paste_pend_len = paste_pend.len();
                        paste_pend_start = Some(Instant::now() - Duration::from_millis(21));
                    }
                    ...
                    if paste_pend.len() >= 3 && (!has_non_ascii || paste_pend.len() >= 20) {
                        ...
                        // Exception: ≥20 chars with non-ASCII is almost certainly a
                        // paste (not IME), so allow it into stage 2.
                        paste_stage2 = true;
                        last_paste_pend_len = paste_pend.len();
                        if input_log_enabled() {
                        ...
                        // 3-19 chars with non-ASCII (IME input) — flush immediately
                        // as normal text to avoid 300ms delay. Larger buffers (≥20)
                        // are caught by the stage2 branch above.
                        if input_log_enabled() {
                    ...
                    // BUT: if chars arrived recently (within 100ms), the paste
                    // is still in progress (e.g. right-click paste via ConPTY
                    // which sends text slowly over ~1s).  Reset the timer.
                    if paste_pend.len() > last_paste_pend_len {
                        last_paste_pend_len = paste_pend.len();
                        paste_pend_start = Some(Instant::now() - Duration::from_millis(21));
                    } else {
                        if input_log_enabled() {
                                ...
                                if tui_active {
                                    ...
                                    if let Some(text) = read_from_system_clipboard() {
                                        ...
                                        if !text.is_empty() {
                                            ...
                                            cmd_batch.push(format!("send-paste {}\n", encoded));
                                            // Suppress paste_pend for 2s — VS Code
                                            // also injects the same text as ConPTY
                                            // key events which would duplicate.
                                            paste_suppress_until = Some(Instant::now() + Duration::from_secs(2));
                                            paste_pend.clear();
                                            paste_pend_start = None;
                                            paste_stage2 = false;
        ...
        #[cfg(windows)]
        {
            if paste_confirmed && !paste_pend.is_empty() {
                // Ctrl+V Release confirms this IS a paste, but ConPTY may
                // still be injecting chars.  Promote to stage2 and let
                // growth detection wait for delivery to finish — flushing
                // now would split the paste into two chunks.
                if input_log_enabled() {
                    input_log("paste", &format!("Ctrl+V Release, promoting {} chars to stage2", paste_pend.len()));
                }
                paste_stage2 = true;
                last_paste_pend_len = paste_pend.len();
                paste_pend_start = Some(Instant::now() - Duration::from_millis(21));
